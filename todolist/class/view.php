...
class view extends config{
    ...
    public function viewData(){
        ...
        foreach ($result as $data){
            ...
            echo "<td>
                  ...
                  <a class='btn btn-secondary btn-sm' href='index2.php?onhold=$data[id]' target='_blank'>Put on Hold</a>
                  </td>";
            echo "</tr>";
        }
        echo "</tbody></table>";
    ...
    public function viewHold(){
        $con = $this->con();
        $sql = "SELECT * FROM `tbl_todolist` WHERE `status` = 'ON HOLD'";
        $data = $con->prepare($sql);
        $data->execute();
        $result = $data->fetchAll(PDO::FETCH_ASSOC);
        echo "<h3 class='mb=4'> On hold task </h3>";
        echo "<table class='table table-dark table-striped table-sm'>";
        echo "<thead>
                <tr>
                    <th>Task Item</th>
